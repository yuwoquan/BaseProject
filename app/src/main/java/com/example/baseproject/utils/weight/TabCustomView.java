...
/**
 * @author Ambrose
 * @date 2019/3/26 4:47 PM
 * @desc
 */
public class TabCustomView  extends HorizontalScrollView {
    ...
    //选中字体大小
    private int mTabSelectTextSize = 0;
    ...
    private int mIndicatorHorPadding = 15;
    private int mIndicatorVerPadding = 15;
    /**
     * indicator在顶部
     */
    ...
    protected OnClickListener mTabOnClickListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (mOnTabClickListener != null) {
                canClick =  mOnTabClickListener.canClick();
            }

            if (!canClick || mSelectAnimator != null || mViewPagerScrollState != ViewPager.SCROLL_STATE_IDLE) {
                return;
            }
    ...
    public TabCustomView(Context context, boolean hasIndicator) {
        this(context, null);
    ...
    public  TabCustomView addTab(Tab item) {
    ...
    public void setIndicatorPadding(int horPadding,int verPadding){
        this.mIndicatorHorPadding = horPadding;
        this.mIndicatorVerPadding = verPadding;
    }

    public void setTabSelectTextSize(int textSize){
        this.mTabSelectTextSize = textSize;
    }
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                if(mTabSelectTextSize > 0){
                    float preSize = mTabSelectTextSize  - (mTabSelectTextSize - mTabTextSize) * animValue;
                    float targetSize = mTabTextSize +  (mTabSelectTextSize - mTabTextSize) * animValue;
                    prevView.setTextSize(preSize);
                    nowView.setTextSize(targetSize);
                }
    ...
    private void layoutIndicator(Tab model, boolean invalidate) {
        ...
        //距离
        mIndicatorRect.left = mIndicatorRect.left + mIndicatorHorPadding;
        mIndicatorRect.right = mIndicatorRect.right - mIndicatorHorPadding;
    ...
    public void updateIndicatorPosition(final int index, float offsetPercent) {
        ...
        if(mTabSelectTextSize > 0){
            float preSize = mTabSelectTextSize  - (mTabSelectTextSize - mTabTextSize) * offsetPercent;
            float targetSize = mTabTextSize +  (mTabSelectTextSize - mTabTextSize) * offsetPercent;
            preView.setTextSize(preSize);
            targetView.setTextSize(targetSize);
        }
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference< TabCustomView> mTabSegmentRef;

        public TabLayoutOnPageChangeListener(TabCustomView tabSegment) {
            mTabSegmentRef = new WeakReference<TabCustomView>(tabSegment);
        }

        @Override
        public void onPageScrollStateChanged(final int state) {
            final TabCustomView tabSegment = mTabSegmentRef.get();
            if (tabSegment != null) {
...
//            this.selectedColor = Color.parseColor("#00A8E1");
        ...
        protected void dispatchDraw(Canvas canvas) {
            ...
            if (mHasIndicator && mIndicatorRect != null) {
                if (mIndicatorTop) {
                    ...
                    mIndicatorRect.top = mIndicatorRect.top + mIndicatorVerPadding;
                    mIndicatorRect.bottom = mIndicatorRect.bottom + mIndicatorVerPadding;
                } else {
